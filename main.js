...
const weekdays = JSON.parse(
    '["07_Sunday","01_Monday","02_Tuesday","03_Wednesday","04_Thursday","05_Friday","06_Saturday"]',
);
...
class Sourceanalytix extends utils.Adapter {
        ...
        try {
                    ...
                    if (customStateArray.rows[index].value) {
                        ...
                        if (customStateArray.rows[index].value[this.namespace]) {
                            ...
                            if (customStateArray.rows[index].value[this.namespace].enabled) {
                                ...
                                this.log.debug(
                                    `SourceAnalytix configuration found but not Enabled, skipping ${stateID}`,
                                );
                            }
                        } else {
            ...
            if (totalFailedStates > 0) {
                ...
                if (totalFailedStates < totalEnabledStates) {
                    this.log.warn(
                        `Partially activated SourceAnalytix for ${totalInitiatedStates} of ${totalEnabledStates} states, check error messages!`,
                    );
                }
            } else {
                this.log.info(
                    `Successfully activated SourceAnalytix for all ${totalInitiatedStates} of ${totalEnabledStates} states, will do my Job until you stop me!`,
                );
            }
    ...
    async buildStateDetailsArray(stateID) {
        ...
        try {
            ...
            if (stateInfo && stateInfo.common && stateInfo.common.custom && stateInfo.common.custom[this.namespace]) {
                ...
                const valueAtDeviceReset =
                    customData.valueAtDeviceReset || customData.valueAtDeviceReset === 0
                        ? customData.valueAtDeviceReset
                        : null;
                ...
                this.log.debug(
                    `[buildStateDetailsArray] cumulativeValue ${JSON.stringify(cumulativeValue)} | valueAtDeviceReset ${JSON.stringify(valueAtDeviceReset)} | valueAtDeviceInit ${JSON.stringify(valueAtDeviceInit)}`,
                );
                ...
                if (
                    !customData.selectedPrice ||
                    customData.selectedPrice === '' ||
                    customData.selectedPrice === 'Choose'
                ) {
                    this.log.error(
                        `No cost type defined for ${stateID}, please Select Type of calculation at state setting`,
                    );
                    initError = true;
                ...
                if (valueAtDeviceReset > cumulativeValue) {
                    ...
                    if (useUnit !== 'W') {
                        this.log.error(
                            `Check settings for ${stateID} ! Known valueAtDeviceReset : (${valueAtDeviceReset}) > known cumulative value (${cumulativeValue}) cannot proceed`,
                        );
                ...
                this.activeStates[stateID] = {
                    stateDetails: {
                        ...
                        deviceResetLogicEnabled:
                            customData.deviceResetLogicEnabled != null
                                ? customData.deviceResetLogicEnabled || true
                                : true,
                ...
                this.log.debug(
                    `[buildStateDetailsArray] completed for ${stateID}: with content ${JSON.stringify(this.activeStates[stateID])}`,
                );
                return true;
            }
        } catch (error) {
    ...
    async initialize(stateID) {
        try {
            ...
            this.log.debug(
                `Defined calculation attributes for ${stateID} : ${JSON.stringify(this.activeStates[stateID])}`,
            );
            ...
            for (const state of basicStates) {
                ...
                if (
                    state === '05_currentYear' &&
                    (stateDetails.consumption || stateDetails.costs) &&
                    (this.config.store_quarters || this.config.store_months || this.config.store_weeks)
                ) {
                    await this.doLocalStateCreate(
                        stateID,
                        `${actualDate.year}.${stateDetails.headCategory}Cumulative`,
                        `${stateDetails.headCategory}Cumulative`,
                        true,
                        false,
                        false,
                    );
                    await this.doLocalStateCreate(
                        stateID,
                        `${actualDate.year}.${stateDetails.financialCategory}Cumulative`,
                        `${stateDetails.financialCategory}Cumulative`,
                        true,
                        false,
                        false,
                        useCurrency,
                    ...
                    state === '05_currentYear' &&
                    !this.config.store_weeks &&
                    !this.config.store_months &&
                    !this.config.store_quarters
                ) {
                    await this.doLocalStateCreate(
                        stateID,
                        `${actualDate.year}.${stateDetails.headCategory}Cumulative`,
            ...
            if (this.config.store_weeks || this.config.store_months || this.config.store_quarters) {
                await this.doLocalStateCreate(
                    stateID,
                    `${actualDate.year}.readingCumulative`,
                    'Cumulative Reading of Year total',
                    true,
                );
            }
     ...
     *
     * @param {string} id - Object ID that changed
     * @param {ioBroker.Object | null | undefined} obj - Object data
     */
    async onObjectChange(id, obj) {
        //ToDo : Verify with test-results if debounce on object change must be implemented
        if (calcBlock) {
            return;
        } // cancel operation if calculation block is activate
        try {
            const stateID = id;
            ...
            if (obj && obj.common) {
                ...
                if (
                    ...
                    this.log.debug(
                        `Object array of SourceAnalytix activated state changed : ${JSON.stringify(obj)} stored config : ${JSON.stringify(this.activeStates)}`,
                    );
                    // const newDeviceName = stateID.split('.').join('__');
                    ...
                    if (!this.activeStates[stateID]) {
                        ...
                        if (this.activeStates[stateID]) {
                            ...
                            this.log.warn(
                                `[Cannot enable SourceAnalytix for ${stateID}, check settings and error messages`,
                            );
                        }
                    } else {
                        ...
                        await this.buildStateDetailsArray(id);
                        this.log.debug(
                            `Active state array after updating configuration of ${stateID} : ${JSON.stringify(this.activeStates)}`,
                        );
                        ...
                        if (this.activeStates[stateID]) {
                            ...
                            this.log.warn(
                                `[Cannot update SourceAnalytix configuration for ${stateID}, check settings and error messages`,
                            );
                        }
                    }
    ...
    onStateChange(id, state) {
        if (calcBlock) {
            return;
        } // cancel operation if global calculation block is activate
        try {
    ...
    async resetStartValues() {
        try {
            const resetDay = new schedule('0 0 * * *', async () => {
                ...
                this.log.debug(
                    `[resetStartValues] Dates current : ${JSON.stringify(actualDate)} | beforeReset ${JSON.stringify(this.activeStates[beforeReset])}`,
                );
                ...
                for (const stateID in this.activeStates) {
                    ...
                    try {
                        if (
                            ...
                            this.activeStates[stateID].calcValues == null ||
                            this.activeStates[stateID].stateDetails == null
                        ) {
                            this.log.error(
                                `Cannot handle Day reset for ${stateID}, check your configuration (error  messages  at adapter start)`,
                            );
                            continue;
                        }
                        ...
                        this.log.debug(
                            `Memory values for ${stateID} before reset : ${JSON.stringify(this.activeStates[stateID])}`,
                        );
                        ...
                        obj.common.custom[this.namespace] = {
                            ...
                            start_quarter:
                                beforeReset.quarter === actualDate.quarter ? stateValues.start_quarter : reading,
                                ...
                                if (this.config.currentYearDays) {
                                    ...
                                    if (stateDetails.consumption) {
                                        switch (stateDetails.headCategory) {
                                            case 'consumed':
                                                await this.setPreviousValues(
                                                    `${stateDetails.deviceName}.currentYear.consumed.currentWeek.${weekdays[x]}`,
                                                    `${stateDetails.deviceName}.currentYear.consumed.previousWeek.${weekdays[x]}`,
                                                );
                                            ...
                                            case 'delivered':
                                                await this.setPreviousValues(
                                                    `${stateDetails.deviceName}.currentYear.delivered.currentWeek.${weekdays[x]}`,
                                                    `${stateDetails.deviceName}.currentYear.delivered.previousWeek.${weekdays[x]}`,
                                                );
                                                await this.setStateAsync(
                                                    `${stateDetails.deviceName}.currentYear.delivered.currentWeek.${weekdays[x]}`,
                                                    {
                                    ...
                                    switch (stateDetails.financialCategory) {
                                        case 'costs':
                                            await this.setPreviousValues(
                                                `${stateDetails.deviceName}.currentYear.costs.currentWeek.${weekdays[x]}`,
                                                `${stateDetails.deviceName}.currentYear.costs.previousWeek.${weekdays[x]}`,
                                            );
                                            await this.setStateAsync(
                                                `${stateDetails.deviceName}.currentYear.costs.currentWeek.${weekdays[x]}`,
                                                {
                                        ...
                                        case 'earnings':
                                            await this.setPreviousValues(
                                                `${stateDetails.deviceName}.currentYear.earnings.currentWeek.${weekdays[x]}`,
                                                `${stateDetails.deviceName}.currentYear.earnings.previousWeek.${weekdays[x]}`,
                                            );
                                            await this.setStateAsync(
                                                `${stateDetails.deviceName}.currentYear.earnings.currentWeek.${weekdays[x]}`,
                                                {
                                    ...
                                    await this.setPreviousValues(
                                        `${stateDetails.deviceName}.currentYear.meterReadings.currentWeek.${weekdays[x]}`,
                                        `${stateDetails.deviceName}.currentYear.meterReadings.previousWeek.${weekdays[x]}`,
                                    );
                                    await this.setStateAsync(
                                        `${stateDetails.deviceName}.currentYear.meterReadings.currentWeek.${weekdays[x]}`,
                                        {
                        ...
                        if (this.config.currentYearPrevious) {
                                ...
                                switch (stateDetails.headCategory) {
                                    case 'consumed':
                                        if (beforeReset.day !== actualDate.day) {
                                            await this.setPreviousValues(
                                                `${stateDetails.deviceName}.currentYear.consumed.01_currentDay`,
                                        ...
                                        if (beforeReset.week !== actualDate.week) {
                                            await this.setPreviousValues(
                                                `${stateDetails.deviceName}.currentYear.consumed.02_currentWeek`,
                                                `${stateDetails.deviceName}.currentYear.consumed.02_previousWeek`,
                                            );
                                        }

                                        if (beforeReset.month !== actualDate.month) {
                                            await this.setPreviousValues(
                                                `${stateDetails.deviceName}.currentYear.consumed.03_currentMonth`,
                                                `${stateDetails.deviceName}.currentYear.consumed.03_previousMonth`,
                                            );
                                        }

                                        if (beforeReset.quarter !== actualDate.quarter) {
                                            await this.setPreviousValues(
                                                `${stateDetails.deviceName}.currentYear.consumed.04_currentQuarter`,
                                                `${stateDetails.deviceName}.currentYear.consumed.04_previousQuarter`,
                                            );
                                        }

                                        if (beforeReset.year !== actualDate.year) {
                                            await this.setPreviousValues(
                                                `${stateDetails.deviceName}.currentYear.consumed.05_currentYear`,
                                                `${stateDetails.deviceName}.currentYear.consumed.05_previousYear`,
                                            );
                                        }
                                    ...
                                    case 'delivered':
                                        if (beforeReset.day !== actualDate.day) {
                                            await this.setPreviousValues(
                                                `${stateDetails.deviceName}.currentYear.delivered.01_currentDay`,
                                                `${stateDetails.deviceName}.currentYear.delivered.01_previousDay`,
                                            );
                                        }

                                        if (beforeReset.week !== actualDate.week) {
                                            await this.setPreviousValues(
                                                `${stateDetails.deviceName}.currentYear.delivered.02_currentWeek`,
                                                `${stateDetails.deviceName}.currentYear.delivered.02_previousWeek`,
                                            );
                                        }

                                        if (beforeReset.month !== actualDate.month) {
                                            await this.setPreviousValues(
                                                `${stateDetails.deviceName}.currentYear.delivered.03_currentMonth`,
                                                `${stateDetails.deviceName}.currentYear.delivered.03_previousMonth`,
                                            );
                                        }

                                        if (beforeReset.quarter !== actualDate.quarter) {
                                            await this.setPreviousValues(
                                                `${stateDetails.deviceName}.currentYear.delivered.04_currentQuarter`,
                                                `${stateDetails.deviceName}.currentYear.delivered.04_previousQuarter`,
                                            );
                                        }

                                        if (beforeReset.year !== actualDate.year) {
                                            await this.setPreviousValues(
                                                `${stateDetails.deviceName}.currentYear.delivered.05_currentYear`,
                                                `${stateDetails.deviceName}.currentYear.delivered.05_previousYear`,
                                            );
                                        }
                                        break;
                            ...
                            switch (stateDetails.financialCategory) {
                                case 'costs':
                                    if (beforeReset.day !== actualDate.day) {
                                        await this.setPreviousValues(
                                            `${stateDetails.deviceName}.currentYear.costs.01_currentDay`,
                                            `${stateDetails.deviceName}.currentYear.costs.01_previousDay`,
                                        );
                                    }

                                    if (beforeReset.week !== actualDate.week) {
                                        await this.setPreviousValues(
                                            `${stateDetails.deviceName}.currentYear.costs.02_currentWeek`,
                                            `${stateDetails.deviceName}.currentYear.costs.02_previousWeek`,
                                        );
                                    }

                                    if (beforeReset.month !== actualDate.month) {
                                        await this.setPreviousValues(
                                            `${stateDetails.deviceName}.currentYear.costs.03_currentMonth`,
                                            `${stateDetails.deviceName}.currentYear.costs.03_previousMonth`,
                                        );
                                    }

                                    if (beforeReset.quarter !== actualDate.quarter) {
                                        await this.setPreviousValues(
                                            `${stateDetails.deviceName}.currentYear.costs.04_currentQuarter`,
                                            `${stateDetails.deviceName}.currentYear.costs.04_previousQuarter`,
                                        );
                                    }

                                    if (beforeReset.year !== actualDate.year) {
                                        await this.setPreviousValues(
                                            `${stateDetails.deviceName}.currentYear.costs.05_currentYear`,
                                            `${stateDetails.deviceName}.currentYear.costs.05_previousYear`,
                                        );
                                    }
                                    break;
                                ...
                                case 'earnings':
                                    if (beforeReset.day !== actualDate.day) {
                                        await this.setPreviousValues(
                                            `${stateDetails.deviceName}.currentYear.earnings.01_currentDay`,
                                            `${stateDetails.deviceName}.currentYear.earnings.01_previousDay`,
                                        );
                                    }

                                    if (beforeReset.week !== actualDate.week) {
                                        await this.setPreviousValues(
                                            `${stateDetails.deviceName}.currentYear.earnings.02_currentWeek`,
                                            `${stateDetails.deviceName}.currentYear.earnings.02_previousWeek`,
                                        );
                                    }

                                    if (beforeReset.month !== actualDate.month) {
                                        await this.setPreviousValues(
                                            `${stateDetails.deviceName}.currentYear.earnings.03_currentMonth`,
                                            `${stateDetails.deviceName}.currentYear.earnings.03_previousMonth`,
                                        );
                                    }

                                    if (beforeReset.quarter !== actualDate.quarter) {
                                        await this.setPreviousValues(
                                            `${stateDetails.deviceName}.currentYear.earnings.04_currentQuarter`,
                                            `${stateDetails.deviceName}.currentYear.earnings.04_previousQuarter`,
                                        );
                                    }

                                    if (beforeReset.year !== actualDate.year) {
                                        await this.setPreviousValues(
                                            `${stateDetails.deviceName}.currentYear.earnings.05_currentYear`,
                                            `${stateDetails.deviceName}.currentYear.earnings.05_previousYear`,
                                        );
                                    }
                                    break;
                            ...
                            if (beforeReset.day !== actualDate.day) {
                                await this.setPreviousValues(
                                    `${stateDetails.deviceName}.cumulativeReading`,
                                    `${stateDetails.deviceName}.currentYear.meterReadings.01_previousDay`,
                                );
                            }
                            ...
                            if (beforeReset.week !== actualDate.week) {
                                await this.setPreviousValues(
                                    `${stateDetails.deviceName}.cumulativeReading`,
                                    `${stateDetails.deviceName}.currentYear.meterReadings.02_previousWeek`,
                                );
                            }
                            ...
                            if (beforeReset.month !== actualDate.month) {
                                await this.setPreviousValues(
                                    `${stateDetails.deviceName}.cumulativeReading`,
                                    `${stateDetails.deviceName}.currentYear.meterReadings.03_previousMonth`,
                                );
                            }
                            ...
                            if (beforeReset.quarter !== actualDate.quarter) {
                                await this.setPreviousValues(
                                    `${stateDetails.deviceName}.cumulativeReading`,
                                    `${stateDetails.deviceName}.currentYear.meterReadings.04_previousQuarter`,
                                );
                            }
                            ...
                            if (beforeReset.year !== actualDate.year) {
                                await this.setPreviousValues(
                                    `${stateDetails.deviceName}.cumulativeReading`,
                                    `${stateDetails.deviceName}.currentYear.meterReadings.05_previousYear`,
                                );
                            }
                            //derelvis end
                        }
        ...
        try {
            if (this.config.currentYearPrevious) {
                ...
                if (currentState && previousState) {
                    ...
                    this.log.debug(
                        `[setPreviousValues] invalid data for currentState  ${currentState} and/or previousState ${previousState} received`,
                    );
                }
            }
        } catch (e) {
    ...
    async doLocalStateCreate(stateID, stateRoot, name, atDeviceRoot, deleteState, isCurrent, forceUnit) {
        this.log.debug(
            `[doLocalStateCreate] ${stateID} | root : ${stateRoot} | name : ${name}) | atDeviceRoot ${atDeviceRoot} | isCurrent : ${isCurrent}`,
        );

        // Check if stateDetails are preset in memory, other wise abort
        if (this.activeStates[stateID] == null || this.activeStates[stateID].stateDetails == null) {
            return;
        }
        this.log.debug(
            `[doLocalStateCreate] stateDetails ${stateID} : ${JSON.stringify(this.activeStates[stateID].stateDetails)}`,
        );
        ...
        try {
            ...
            if (atDeviceRoot) {
                ...
                if (!deleteState && stateDetails.consumption) {
                    switch (stateDetails.headCategory) {
                        case 'consumed':
                            await this.localSetObject(
                                `${stateDetails.deviceName}.${dateRoot}.consumed.${stateRoot}`,
                                commonData,
                            );
                            await this.localDeleteState(
                                `${stateDetails.deviceName}.${dateRoot}.delivered.${stateRoot}`,
                            );
                            break;
                ...
                if (!deleteState && stateDetails.meter_values) {
                    ...
                    if (!basicStates.includes(stateRoot)) {
                        await this.localSetObject(
                            `${stateDetails.deviceName}.${dateRoot}.meterReadings.${stateRoot}`,
                            commonData,
                        );
                    }
                ...
                if (!deleteState && stateDetails.costs) {
                    ...
                    switch (stateDetails.financialCategory) {
                        case 'costs':
                            ...
                            await this.localSetObject(
                                `${stateDetails.deviceName}.${dateRoot}.costs.${stateRoot}`,
                                commonData,
                            );
                        ...
                        case 'earnings':
                            // await this.ChannelCreate(device, head_category, head_category);
                            await this.localSetObject(
                                `${stateDetails.deviceName}.${dateRoot}.earnings.${stateRoot}`,
                                commonData,
                            );
    ...
    async calculationHandler(stateID, stateVal) {
        try {
            ...
            if (!stateVal) {
                this.log.error(
                    `Input value for ${stateID} with ${JSON.stringify(stateVal)} is null or undefined, cannot continue calculation`,
                );
                return;
            }
            ...
            if (reading === null || reading === undefined) {
                this.log.error(
                    `[calculationHandler] reading incorrect after conversion contact DEV and provide these info | Reading : ${JSON.stringify(reading)} | start reading ${JSON.stringify(stateVal)} | stateDetails ${JSON.stringify(stateDetails)}`,
                );
                return;
            }
            ...
            this.log.debug(
                `[calculationHandler] Reading value ${reading} before exponent multiplier | currentExponent : ${JSON.stringify(currentExponent)} | targetExponent : ${JSON.stringify(targetExponent)}`,
            );
            ...
            if (reading === null || reading === undefined) {
                this.log.error(
                    `[calculationHandler] reading incorrect after Exponent conversion contact DEV and provide these info | Reading : ${JSON.stringify(reading)} | start reading ${JSON.stringify(stateVal)} | currentExponent ${currentExponent} | targetExponent ${targetExponent} | stateDetails ${stateDetails}`,
                );
                return;
            }

            this.log.debug(
                `[calculationHandler] reading value ${reading} after exponent multiplier : ${JSON.stringify(targetExponent)}`,
            );
            ...
            if (calcValues.valueAtDeviceReset == null && currentCath !== 'Watt') {
                ...
                reading + calcValues.valueAtDeviceReset >= calcValues.cumulativeValue &&
                currentCath !== 'Watt'
            ) {
                this.log.debug(
                    `[calculationHandler] New reading ${reading} bigger than stored value ${calcValues.valueAtDeviceInit} processing normally`,
                );
                this.log.debug(
                    `[calculationHandler] Adding ${reading} to stored value ${this.activeStates[stateID].calcValues.valueAtDeviceReset}`,
                );
                ...
                this.log.debug(
                    `[calculationHandler] Calculation outcome ${reading} valueAtDeviceReset ${this.activeStates[stateID].calcValues.valueAtDeviceReset}`,
                );
                ...
                if (
                    ...
                    reading + calcValues.valueAtDeviceReset + stateDetails.threshold < calcValues.cumulativeValue
                ) {
                    this.log.warn(
                        `Device reset detected for ${stateID} store current cumulatedReading ${calcValues.cumulativeValue} as valueAtDeviceReset (previous valueAtDeviceReset : ${calcValues.valueAtDeviceReset})`,
                    );
                    await initiateState();
                } else {
                ...
                this.log.error(
                    `[calculationHandler] unforeseen situation for ${stateID} with value ${reading}, please send this to developer | reading : ${reading} | calcvalues : ${JSON.stringify(stateDetails)}`,
                );
            }
            ...
            this.log.debug(
                `[calculationHandler] PreviousCalculationRounded for ${stateID} : ${JSON.stringify(previousCalculationRounded[stateID])}`,
            );
            ...
            if (stateDetails.meter_values === true) {
                ...
                if (readingRounded) {
                    ...
                    // Quarter
                    if (storeSettings.storeQuarters) {
            ...
            if (stateDetails.consumption) {
                ...
                if (this.config.store_weeks || this.config.store_months || this.config.store_quarters) {
                    await this.setStateChangedAsync(
                        `${this.namespace}.${stateDetails.deviceName}.${actualDate.year}.${stateDetails.headCategory}Cumulative`,
                        {
                            val: calculationRounded.consumedYear,
                            ack: true,
                        },
            ...
            if (stateDetails.costs) {
                ...
                if (this.config.store_weeks || this.config.store_months || this.config.store_quarters) {
                    await this.setStateChangedAsync(
                        `${this.namespace}.${stateDetails.deviceName}.${actualDate.year}.${stateDetails.financialCategory}Cumulative`,
                        {
                            val: calculationRounded.priceYear,
                            ack: true,
                        },
            ...
            this.log.debug(
                `[calculationHandler] Meter Calculation executed consumed data for ${stateID} : ${JSON.stringify(calculationRounded)}`,
            );
        } catch (error) {
            this.errorHandling(
    ...
    async wattToWattHour(stateID, value) {
        try {
            ...
            this.log.debug(
                `[wattToWattHour] Watt to kWh, current reading : ${value.val} previousReading : ${JSON.stringify(calcValues)}`,
            );
            ...
            this.log.debug(
                `[wattToWattHour] ${stateID} Watt to kWh outcome : ${JSON.stringify(this.activeStates[stateID].calcValues)}`,
            );
            return calckWh;
        } catch (error) {
